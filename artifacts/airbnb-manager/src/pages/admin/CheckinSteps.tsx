import { 
  useListCheckinSteps, 
  useCreateCheckinStep, 
  useUpdateCheckinStep, 
  useDeleteCheckinStep,
  getListCheckinStepsQueryKey
...
export default function CheckinSteps() {
  const { data: steps, isLoading } = useListCheckinSteps();
  const createStep = useCreateCheckinStep();
  const updateStep = useUpdateCheckinStep();
  const deleteStep = useDeleteCheckinStep();
  ...
  const handleDelete = (id: number) => {
    if (confirm("Tem certeza que deseja excluir este passo?")) {
      deleteStep.mutate(
        ...
        {
          onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: getListCheckinStepsQueryKey() });
    ...
    <AdminLayout>
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <h1 className="text-3xl font-serif font-bold text-foreground">Passos de Check-in</h1>
            <p className="text-muted-foreground mt-2">
              Instruções passo a passo para a chegada dos hóspedes.
            </p>
          </div>
              ...
              <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input placeholder="Ex: Pegue a chave na portaria" {...field} />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                    )}
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem className="flex items-center justify-between rounded-xl border p-4 bg-amber-50 border-amber-200">
                        <div className="space-y-0.5">
                          <FormLabel className="text-amber-900 font-semibold flex items-center gap-1.5">
                            <ShieldCheck className="h-4 w-4" />
                            Exige confirmação do hóspede
                          </FormLabel>
                          <FormDescription className="text-amber-700 text-xs">
                            O hóspede terá que marcar este passo antes de liberar o botão "Estou pronto!".
                          </FormDescription>
                        </div>
                        <FormControl>
                          <Switch
                            ...
                            data-testid="toggle-requires-confirmation"
                          />
                        </FormControl>
                      </FormItem>
                    )}
        ...
        {sortedSteps.length === 0 ? (
          <div className="text-center py-12 bg-muted/30 rounded-lg border border-dashed">
            <p className="text-muted-foreground">Nenhum passo de check-in cadastrado.</p>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                      ...
                      {step.requiresConfirmation && (
                        <span className="inline-flex items-center gap-1 text-[10px] font-bold uppercase tracking-wide text-amber-700 bg-amber-100 border border-amber-200 rounded-full px-2 py-0.5">
